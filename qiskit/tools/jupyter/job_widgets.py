...
def create_job_widget(watcher, job, backend, status='', queue_pos=None, msg=''):
    ...
    id_label = widgets.HTML(value=f"{job_id}",
                            layout=widgets.Layout(width='190px'))
    backend_label = widgets.HTML(value=f"{backend}",
                                 layout=widgets.Layout(width='145px'))
    status_label = widgets.HTML(value=f"{status}",
                                layout=widgets.Layout(width='95px'))
    if queue_pos is None:
        queue_pos = '-'
    else:
        queue_pos = str(queue_pos)
    queue_label = widgets.HTML(value=f"{queue_pos}",
                               layout=widgets.Layout(width='70px'))

    msg_label = widgets.HTML(value=f"<p style=white-space:nowrap;>{msg}</p>",
